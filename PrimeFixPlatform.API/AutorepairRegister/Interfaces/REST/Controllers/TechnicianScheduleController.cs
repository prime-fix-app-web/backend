...
: ControllerBase
{
    ...
    [SwaggerResponse(StatusCodes.Status500InternalServerError, 
        "Internal server error", 
        typeof(InternalServerErrorResponse))]
    public async Task<IActionResult> GetTechnicianScheduleById([FromRoute] int technician_schedule_id)
    {
        var getTechnicianScheduleByIdQuery = new GetTechnicianScheduleByIdQuery(technician_schedule_id);
        var technicianSchedule = await technicianScheduleQueryService.Handle(getTechnicianScheduleByIdQuery);
    ...
    /// </returns>
    [HttpPut("{technician_schedule_id}")]
    [SwaggerOperation(
    ...
    [SwaggerResponse(StatusCodes.Status500InternalServerError, 
        "Internal server error", 
        typeof(InternalServerErrorResponse))]
    public async Task<IActionResult> UpdateTechnicianSchedule([FromRoute] int technician_schedule_id, [FromBody] UpdateTechnicianScheduleRequest request)
    {
        var updateTechnicianScheduleCommand = TechnicianScheduleAssembler.ToCommandFromRequest(request, technician_schedule_id);
    ...
    /// </returns>
    [HttpDelete("{technician_schedule_id}")]
    [SwaggerOperation(
    ...
    [SwaggerResponse(StatusCodes.Status500InternalServerError, 
        "Internal server error", 
        typeof(InternalServerErrorResponse))]
    public async Task<IActionResult> DeleteTechnicianSchedule([FromRoute] int technician_schedule_id)
    {
        var deleteTechnicianScheduleCommand = new DeleteTechnicianScheduleCommand(technician_schedule_id);
